...
export const commitTransaction = (config: TBKConfig): Endpoint<CommitTransactionRequest, CommitTransactionResponse> => {
  return async (token: CommitTransactionRequest, logger: Logger): Promise<CommitTransactionResponse> => {
    const requestArgs = {
    ...
    return result as unknown as CommitTransactionResponse;
  };
}
